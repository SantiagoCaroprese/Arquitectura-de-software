...
namespace DataAccess
{
    public class DataAccessService : DataAccessor.DataAccessorBase
    {
        ...
        {
            MasterCard card = JsonSerializer.Deserialize<MasterCard>(request.Card);
            bool save = false;
        ...
        static bool CheckForExistingCard(string cardNumber)
        {
            ...
            {
                var os = context.MasterCard.Where(o => o.Number.Equals(cardNumber));
                if (os.Count() > 0)
                {
                    exists = true;
                }
        ...
        static MasterCard GetCard(string cardNumber)
        {
            MasterCard card = null;
            using (var dbContext = new CardsContext())
            {
                card = dbContext.MasterCard.Where(o => o.Number.Equals(cardNumber)).First();
            }
            return card;
        }
    }
